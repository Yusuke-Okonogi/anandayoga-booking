'use client';

import { useState } from 'react';
import { format } from 'date-fns';
import ModalPortal from './ModalPortal';

export type News = {
  id: string;
  title: string;
  publishedAt: string;
  thumbnail?: {
    url: string;
  };
  content?: string;
};

type Props = {
  newsList: News[];
};

export default function NewsSection({ newsList }: Props) {
  const [selectedNews, setSelectedNews] = useState<News | null>(null);

  // モーダルを開く
  const openModal = (news: News) => {
    setSelectedNews(news);
    document.body.style.overflow = 'hidden';
  };

  // モーダルを閉じる
  const closeModal = () => {
    setSelectedNews(null);
    document.body.style.overflow = '';
  };

  return (
    <section className="py-16 px-4 bg-[#F9F8F6]">
      <div className="text-center mb-8">
        <h3 className="text-[#EEA51A] font-bold text-lg mb-1 font-sans">News</h3>
        <h2 className="text-xl font-bold tracking-widest text-stone-700">お知らせ</h2>
      </div>
      
      {/* ニュース一覧 */}
      {newsList.length === 0 ? (
        <div className="text-center text-xs text-stone-400 py-4">お知らせはありません</div>
      ) : (
        <div className="flex gap-4 overflow-x-auto pb-4 px-2 snap-x scrollbar-hide">
           {newsList.map((news) => (
             <div 
                key={news.id} 
                onClick={() => openModal(news)}
                className="min-w-[200px] bg-white p-3 rounded-xl shadow-sm border border-stone-100 snap-center block hover:shadow-md transition cursor-pointer"
             >
                <div className="aspect-square bg-stone-100 mb-3 rounded-lg overflow-hidden relative">
                  {news.thumbnail ? (
                    <img 
                      src={news.thumbnail.url} 
                      alt={news.title} 
                      className="object-cover w-full h-full" 
                    />
                  ) : (
                    <div className="absolute inset-0 flex items-center justify-center text-stone-300 text-xs">No Image</div>
                  )}
                </div>
                <div className="text-[10px] text-stone-400 font-bold mb-1">
                  {format(new Date(news.publishedAt), 'yyyy.MM.dd')}
                </div>
                <div className="text-xs font-bold text-stone-700 line-clamp-2">
                  {news.title}
                </div>
                <div className="mt-3 text-[10px] border border-stone-300 px-3 py-1 rounded w-full text-stone-500 text-center hover:bg-stone-50 transition">
                  詳しくみる
                </div>
             </div>
           ))}
        </div>
      )}

      {/* ニュース詳細モーダル (Portal使用) */}
      {selectedNews && (
        <ModalPortal>
          <div 
            className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200"
            onClick={closeModal}
          >
            <div 
              className="bg-white rounded-2xl w-full max-w-md max-h-[85vh] flex flex-col shadow-2xl relative overflow-hidden"
              onClick={(e) => e.stopPropagation()}
            >
              {/* 閉じるボタン (右上) */}
              <button 
                onClick={closeModal}
                className="absolute top-4 right-4 bg-stone-100 rounded-full p-2 text-stone-500 hover:text-stone-800 shadow-sm z-20 transition hover:scale-105"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
              </button>

              {/* スクロール可能なコンテンツエリア */}
              <div className="overflow-y-auto flex-1 overscroll-contain p-6 md:p-8">
                  
                  {/* 1. 日付 */}
                  <div className="mb-2">
                    <p className="text-sm text-[#EEA51A] font-bold">
                      {format(new Date(selectedNews.publishedAt), 'yyyy.MM.dd')}
                    </p>
                  </div>

                  {/* 2. タイトル */}
                  <h3 className="text-xl font-bold text-stone-800 leading-snug mb-6 border-b border-stone-100 pb-4">
                    {selectedNews.title}
                  </h3>

                  {/* 3. サムネイル (あれば表示) */}
                  {selectedNews.thumbnail && (
                    <div className="w-full mb-6 rounded-xl overflow-hidden border border-stone-100">
                      <img 
                        src={selectedNews.thumbnail.url} 
                        alt={selectedNews.title} 
                        className="w-full h-auto object-cover"
                      />
                    </div>
                  )}

                  {/* 4. 本文 */}
                  <div 
                    className="prose prose-stone prose-sm max-w-none text-sm leading-loose text-stone-600 space-y-4"
                    dangerouslySetInnerHTML={{ __html: selectedNews.content || '' }}
                  />
              </div>

              {/* 固定フッター (閉じるボタン) */}
              <div className="p-4 border-t border-stone-100 bg-white text-center shrink-0 z-10">
                <button 
                  onClick={closeModal}
                  className="text-sm font-bold text-stone-500 hover:text-stone-800 px-10 py-3 rounded-full border border-stone-300 hover:bg-stone-50 transition w-full sm:w-auto"
                >
                  閉じる
                </button>
              </div>
            </div>
          </div>
        </ModalPortal>
      )}
    </section>
  );
}